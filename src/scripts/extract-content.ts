...
function extractLinks(htmlContent: string, baseUrl: string): PageLink[] {
...
function parse(document: Document): ExtractionResult | false {
  ...
  // Extract links from the article HTML content
  const links = extractLinks(article.content, document.location.href);

  // Extract navigation context from the original document (before Readability processing)
